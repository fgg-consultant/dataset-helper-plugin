...
COPY --chown=$PLUGIN_BUILD_UID:$PLUGIN_BUILD_GID ./plugins/dataset_helper_plugin/requirements/dev.txt /tmp/plugin-dev-requirements.txt
RUN . /climweb/venv/bin/activate && pip3 install -r /tmp/plugin-dev-requirements.txt

COPY --chown=$PLUGIN_BUILD_UID:$PLUGIN_BUILD_GID ./plugins/dataset_helper_plugin/ $CLIMWEB_PLUGIN_DIR/dataset_helper_plugin/
RUN . /climweb/venv/bin/activate && /climweb/plugins/install_plugin.sh --folder $CLIMWEB_PLUGIN_DIR/dataset_helper_plugin --dev
